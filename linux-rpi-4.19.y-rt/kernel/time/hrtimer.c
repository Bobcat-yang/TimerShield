...
static int enqueue_hrtimer_with_prio(struct hrtimer *timer,
			   struct hrtimer_clock_base *base, int prio,enum hrtimer_mode mode)
{
	debug_activate(timer,mode);
...
retry:
	...
	was_higher_prio = __hrtimer_run_queues(cpu_base, now, flags, HRTIMER_ACTIVE_HARD);
    if (was_higher_prio) {
		hrtimer_update_next_timer(cpu_base, NULL);
		tick_program_event(cpu_base->expires_next, 1);
...
/**
 ...
 void hrtimer_context_switch_timershield()
 {
 	...
 	for (; active; base++, active >>= 1) {
 		...
 		if (next->expires < to_reprog->expires)
 			to_reprog = next;
 	}
 	if (raise)
 		raise_softirq_irqoff(HRTIMER_SOFTIRQ);
 	...
 	if (timer != cpu_base->next_timer) {
 		...
 		cpu_base->expires_next= to_reprog->expires;
